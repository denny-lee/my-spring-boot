...
public class RubicsCube {
    ...
    public RubicsCube(List<Cube> cbs) {
        cube = new Block[8];
        cbs.sort(Comparator.comparing(Cube::getPos));
        int idx = 0;
        for (Cube cb : cbs) {
            int[] vals = getValueByColor(cb.getColors());
            cube[idx] = new Block(idx, vals[0], vals[1], vals[2]);
            idx++;
        }
        System.out.println("-------init-------");
        printStatus();
    ...
    private int[] getValueByColor(List<String> colors) {
        int[] v = new int[3];
        int val = -1;
...
//        System.out.println("Done!");
        ...
        String key = sb.toString().length()>0?sb.toString().substring(1):"";
